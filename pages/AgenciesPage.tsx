...
const AgenciesPage: React.FC<AgenciesPageProps> = ({ lang }) => {
  ...
  const loadAgencies = async () => {
    try {
      ...
      setError(lang === 'fr' ? 'Erreur lors du chargement des agences' : 'خطأ في تحميل الوكالات');
    } finally {
      setIsLoading(false);
    }
  };

  const handleSaveAgency = async (agencyData: Agency | Omit<Agency, 'id'>) => {
    try {
      setIsLoading(true);
      setError(null);
      
      if (editingAgency && 'id' in agencyData) {
        const updated = await dataService.updateAgency(editingAgency.id, agencyData);
        const newAgencies = agencies.map(a => a.id === updated.id ? updated : a);
        setAgencies(newAgencies);
      } else {
        const created = await dataService.createAgency(agencyData as Omit<Agency, 'id'>);
        const newAgencies = [created, ...agencies];
        setAgencies(newAgencies);
      }
  ...
  const handleDeleteAgency = async (agency: Agency) => {
    try {
      setIsLoading(true);
      setError(null);
      await dataService.deleteAgency(agency.id);
  ...
  const DeleteModal = ({ agency }: { agency: Agency }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl animate-scale-in p-12 text-center">
        ...
        <p className="text-gray-500 font-bold mb-10 leading-relaxed">
          {agency.name}<br/>
          <span className="text-xs uppercase tracking-widest opacity-60 font-black">{agency.address}</span>
        </p>
        <div className="flex gap-4">
          <button onClick={() => setActiveModal({ type: null, agency: null })} className="flex-1 py-5 bg-gray-100 rounded-[1.5rem] font-black text-gray-500 hover:bg-gray-200 transition-all uppercase text-xs tracking-widest">{currentT.cancel}</button>
          <button onClick={() => handleDelete(agency.id)} className="flex-1 py-5 bg-red-600 rounded-[1.5rem] font-black text-white hover:bg-red-700 transition-all shadow-xl shadow-red-100 uppercase text-xs tracking-widest">{currentT.delete}</button>
        </div>
      </div>
    </div>
  );
          ...
          <div className="p-8 md:p-14">
            <div className="flex items-center justify-between mb-12 border-b border-gray-50 pb-12">
              <h2 className="text-5xl font-black text-gray-900 flex items-center gap-6">
                ...
                {editingAgency ? currentT.editTitle : currentT.createTitle}
              </h2>
            ...
            <form className="space-y-10" onSubmit={handleSave}>
              <div className="space-y-8">
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 px-2">{currentT.agencyName}</label>
                  <input 
                    name="name"
                    type="text" 
                    defaultValue={editingAgency?.name} 
                    className="w-full px-8 py-6 bg-gray-50 border-2 border-transparent focus:bg-white focus:border-blue-500 rounded-[2.5rem] outline-none font-black text-xl transition-all shadow-inner" 
                    placeholder="Ex: DriveFlow Alger Centre" 
                    required 
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 px-2">{currentT.address}</label>
                  <input 
                    name="address"
                    type="text" 
                    defaultValue={editingAgency?.address} 
                    className="w-full px-8 py-6 bg-gray-50 border-2 border-transparent focus:bg-white focus:border-blue-500 rounded-[2.5rem] outline-none font-black transition-all shadow-inner" 
                    placeholder="Ex: 12 Rue Didouche Mourad, Alger" 
                    required 
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 px-2">{currentT.phone}</label>
                  <input 
                    name="phone"
                    type="tel" 
    ...
    <div className={`p-4 md:p-8 ${isRtl ? 'font-arabic text-right' : ''}`}>
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between gap-10 mb-16 ${isRtl ? 'flex-row-reverse' : ''}`}>
        <div>
          ...
          <div className="flex items-center gap-3 text-gray-500 font-bold">
            ...
            {filteredAgencies.length} {currentT.totalAgencies}
          </div>
        </div>
      ...
      {filteredAgencies.length > 0 ? (
          ...
          {filteredAgencies.map((agency) => (
            <div key={agency.id} className="group bg-white rounded-[4rem] shadow-[0_30px_80px_-15px_rgba(0,0,0,0.08)] border border-gray-100 overflow-hidden hover:shadow-[0_40px_100px_-20px_rgba(59,130,246,0.15)] hover:-translate-y-4 transition-all duration-700 flex flex-col">
              <div className="p-10 flex flex-col h-full">
                <div className="w-20 h-20 bg-gray-50 rounded-[2rem] flex items-center justify-center text-4xl mb-8 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 shadow-inner">
                  🏢
                </div>
                
                <h3 className="text-2xl font-black text-gray-900 group-hover:text-blue-600 transition-colors leading-tight mb-4">
                  {agency.name}
                </h3>
        ...
        <div className="bg-white rounded-[3rem] p-20 text-center border-2 border-dashed border-gray-100">
          ...
          <p className="text-xl font-black text-gray-400 uppercase tracking-widest">{currentT.noAgencies}</p>
        </div>
      )}

      {/* Active Modals */}
      {activeModal.type === 'delete' && activeModal.agency && <DeleteModal agency={activeModal.agency} />}
    </div>
  );
};
